...
public class SectorInside extends Activity {
    ...
    private void ProcessButtonInput(Button b, int numTable, View v)
    {
		...
		{
			b.setText(numTable + " - Occupied"); //CHANGE THIS -> TODO: UPDATE DATABASE
            Intent tableNO = new Intent(v.getContext(), DetailsOfOrder.class);
		...
		{
			Intent tableOD = new Intent(v.getContext(), DetailsOfOrder.class);
			tableOD.putExtra("SectorNum", title.getText().toString());
			startActivity(tableOD);
		}
		else if(CheckSubString(b.getText().toString(), "Reserved") != -1)
		{
			// TODO: ARE YOU SURE??
			b.setText(numTable + " - Available"); // CHANGE THIS -> TODO: UPDATE DATABASE
		}
    }
